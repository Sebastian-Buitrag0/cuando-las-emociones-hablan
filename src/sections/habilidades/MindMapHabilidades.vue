...
const dp = [-90, -30, 30, 90, 150, 210].map(a => polar(CX, CY, R, a));
...
const habilidades: Habilidad[] = [
  ...
  { id: "toma-decisiones", title: "Toma De Decisiones", shortTitle: "Toma de\nDecisiones", emoji: "🎯", emojiImg: `${EMOJI_BASE}/Direct%20hit/3D/direct_hit_3d.png`, gf: "#48bb78", gt: "#276749", color: "hsl(145,63%,42%)", conector: "Integra todo lo anterior: decidir bien requiere reconocer lo que sientes, regularlo, considerar a otros y mantenerte motivado.", description: "Evaluar opciones, anticipar consecuencias y elegir el camino que se alinea con tus valores y metas personales.", señales: ["Evitas tomar decisiones por miedo a equivocarte o a decepcionar a alguien.", "Dejas que otros decidan por ti en situaciones importantes de tu vida.", "Después de decidir, entras en un ciclo de duda y arrepentimiento constante."], tips: ["Antes de decidir, escribe las opciones posibles y una consecuencia de cada una.", "Consulta a alguien de confianza, pero recuerda que la decisión final es tuya.", "Acepta que equivocarse es parte del aprendizaje: ninguna decisión es definitiva.", "Distingue lo urgente de lo importante para no decidir desde el pánico."], hasSubSkills: false, practica: { accion: "Mapa de decisión en 3 pasos", pasos: ["Escribe una decisión que tengas pendiente, pequeña o grande.", "Lista 2 o 3 opciones posibles y anota 1 consecuencia de cada una.", "Elige la opción con la que te sientas más en paz, no la más fácil."] }, x: dp[5].x, y: dp[5].y, my: 788 },
];
            ...
            <template v-else>
              <div v-if="selected.señales.length" class="rounded-2xl bg-surface/85 border border-border/60 p-4 mb-4">
                <p class="text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-1.5 text-muted-foreground">
                  <AlertCircle class="w-3.5 h-3.5 flex-shrink-0" /> Aspectos Que Aún Se Pueden Fortalecer
                </p>
                <ul class="space-y-2">
